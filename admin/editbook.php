...
$connection=mysqli_connect("localhost","root","");
    ...
    $category="";
    $price="";
    $query="SELECT b.id AS book_no,b.name AS book_name,a.name AS author_name,b.`purchase date` AS purchase_date,
            c.name AS category_name,b.price AS price FROM books b JOIN 
            authors a ON b.`author id` = a.id JOIN category c ON b.category_id = c.id WHERE  b.id = $_GET[bno]";
...
<center><h3>Edit book details</h3></center>
           ...
           <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="authorname" value="<?php echo $author; ?>">
...
if(isset($_POST['edit'])){
   ...
   $row=mysqli_fetch_assoc($query_run);
   $category=$row['id'];
    $query = "update books set name='$_POST[name]',`author id`=$authorid ,`purchase date`='$_POST[date]',category_id= $category , price= $_POST[price] where id=$_GET[bno]";
    $query_run = mysqli_query($connection, $query);
        
        
if ($query_run) {
    echo "<script>alert('Book details updated successfully!'); window.location.href = 'manage book.php';</script>";
   
} else {
    echo "<script>alert('Update failed due to some error.... '); window.location.href = 'manage book.php';</script>";
